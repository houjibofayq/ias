...
#define ESP_MODEM_LINE_BUFFER_SIZE (TGLS_MODEM_UART_RX_BUFFER_SIZE / 2)
...
modem_dte_t *esp_modem_dte_init(const esp_modem_dte_config_t *config)
{
    ...
    res = uart_driver_install(esp_dte->uart_port, TGLS_MODEM_UART_RX_BUFFER_SIZE, TGLS_MODEM_UART_TX_BUFFER_SIZE,
                              TGLS_MODEM_UART_EVENT_QUEUE_SIZE, &(esp_dte->event_queue), 0);
    ...
    if (config->flow_control == MODEM_FLOW_CONTROL_HW) {
        res = uart_set_pin(esp_dte->uart_port, TGLS_MODEM_UART_MODEM_TX_PIN, TGLS_MODEM_UART_MODEM_RX_PIN,
                           TGLS_MODEM_UART_MODEM_RTS_PIN, TGLS_MODEM_UART_MODEM_CTS_PIN);
    } else {
        res = uart_set_pin(esp_dte->uart_port, TGLS_MODEM_UART_MODEM_TX_PIN, TGLS_MODEM_UART_MODEM_RX_PIN,
    ...
    res |= uart_pattern_queue_reset(esp_dte->uart_port, TGLS_MODEM_UART_PATTERN_QUEUE_SIZE);
    ...
    BaseType_t ret = xTaskCreate(uart_event_task_entry,             //Task Entry
                                 ...
                                 TGLS_MODEM_UART_EVENT_TASK_STACK_SIZE, //Task Stack Size(Bytes)
                                 esp_dte,                           //Task Parameter
                                 TGLS_MODEM_UART_EVENT_TASK_PRIORITY,   //Task Priority
...
esp_err_t esp_modem_start_ppp(modem_dte_t *dte)
{
    ...
    esp_modem_dte_t *esp_dte = __containerof(dte, esp_modem_dte_t, parent);
    /* Set PDP Context */
    MODEM_CHECK(dce->define_pdp_context(dce, 1, "IP", TGLS_MODEM_APN) == ESP_OK, "set MODEM APN failed", err);
    /* Enter PPP mode */
