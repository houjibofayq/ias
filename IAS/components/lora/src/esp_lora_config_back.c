...
#define LORA_SPI_RST_GPIO        32
#define LORA_SPI_CS_GPIO         15
#define LORA_SPI_SCK_GPIO        14
#define LORA_SPI_MISO_GPIO       13
#define LORA_SPI_MOSI_GPIO       12
...
lora_write_reg(int reg, int val)
{
   ...
   gpio_set_level(LORA_SPI_CS_GPIO, 0);
   spi_device_transmit(__spi, &t);
   gpio_set_level(LORA_SPI_CS_GPIO, 1);
}
...
lora_reset(void)
{
   gpio_set_level(LORA_SPI_RST_GPIO, 0);
   vTaskDelay(pdMS_TO_TICKS(1));
   gpio_set_level(LORA_SPI_RST_GPIO, 1);
...
lora_init(void)
{
    ...
    */
   gpio_pad_select_gpio(LORA_SPI_RST_GPIO);
   gpio_set_direction(LORA_SPI_RST_GPIO, GPIO_MODE_OUTPUT);
   gpio_pad_select_gpio(LORA_SPI_CS_GPIO);
   gpio_set_direction(LORA_SPI_CS_GPIO, GPIO_MODE_OUTPUT);

   spi_bus_config_t bus = {
      .miso_io_num = LORA_SPI_MISO_GPIO,
      .mosi_io_num = LORA_SPI_MOSI_GPIO,
      .sclk_io_num = LORA_SPI_SCK_GPIO,
      .quadwp_io_num = -1,
      .quadhd_io_num = -1,
      .max_transfer_sz = 0
   };
